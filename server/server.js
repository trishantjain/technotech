...
// // ================ CHECKING SERVER BLOCKING ================
// const { performance, monitorEventLoopDelay } = require('perf_hooks');

// const h = monitorEventLoopDelay({ resolution: 20 });
// h.enable();
// setInterval(() => {
//   console.log(`[ELAG] mean=${(Number(h.mean) / 1e6).toFixed(2)}ms max=${(Number(h.max) / 1e6).toFixed(2)}ms`);
//   h.reset();
// }, 5000);
...
//     console.log(`[eMS_LOGS] Received data event (${data.length} bytes)`);
//     let packetCount = 0;
//     const dataStart = Date.now();
//     console.log(`[LOG] Received packet (${data.length} bytes) from ${clientInfo} at ${new Date(dataStart).toISOString()}`);
...
//         if (eMS_LOGS) console.log(`[BUFFER] pushed reading; readingBuffer.length=${readingBuffer.length}`);
//         if (eMS_LOGS) console.log(`[eMS_LOGS] Finished parsing packet for MAC: ${mac}`);
